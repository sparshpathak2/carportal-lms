...
import { getAllUsers } from "@/features/users/api/user"
...
export default function Page() {
    ...
    const {
        ...
        queryKey: ["users"], // 🧩 include filters in query key for auto refetch
        // queryFn: () => getLeadsWithFilters(filters), // ✅ pass filters to API
        queryFn: () => getAllUsers(), // ✅ pass filters to API
    ...
    const filteredData = React.useMemo(() => {
        ...
        if (filters.status?.length) {
            results = results.filter((user) => {
                ...
                const leadStatus = typeof user.status === "object" ? user.status.name : user.status;
        ...
        return results;
    }, [allUsers, filters, filtersActive]);
        ...
        <div className="flex flex-1 flex-col gap-2">
            ...
            <div className="sticky top-0 z-50 bg-white flex flex-col gap-2 border-b">
                ...
                <div className="flex border-b px-2">
                    <div
                        className={`font-semibold p-2 cursor-pointer border-b-2 ${activeTab === "all_users" ? "border-blue-600" : "border-transparent"
                            }`}
